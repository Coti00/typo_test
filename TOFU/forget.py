...
def main(cfg):
    ...
    if local_rank == 0:
        os.environ['WANDB_API_KEY'] = 'abf3daa9a6416e1074c27ca58535013a8bea3c0b'
        wandb.init(
            project='tnpo',
            entity='chaewon0510-chaewon',
            name=f"{cfg.forget_loss}_{cfg.split}_seed{cfg.seed}",
            config={
    ...
    training_args = transformers.TrainingArguments(
            ...
            weight_decay = cfg.weight_decay,
            report_to="wandb" if local_rank == 0 else "none",  # Enable wandb logging on main process
    )
    ...
    if path_found:
        ...
        model = AutoModelForCausalLM.from_pretrained(cfg.model_path, attn_implementation=attn_implementation, torch_dtype=torch.bfloat16, trust_remote_code = True)
        oracle_model = AutoModelForCausalLM.from_pretrained(cfg.model_path, attn_implementation=attn_implementation, torch_dtype=torch.bfloat16, trust_remote_code = True)

    else:
        print("Loading after merge and unload")
        attn_implementation = "flash_attention_2" if model_cfg["flash_attention2"]=="true" else "eager"
        model = AutoModelForCausalLM.from_pretrained(model_id, attn_implementation=attn_implementation, torch_dtype=torch.bfloat16, device_map=device_map)
