...
def convert_answer_only_to_model_format(tokenizer, max_length, answer, model_configs, model_family=None):
    """
    Answer만 사용하는 버전 (Question 완전 제거)
    Influence Function 계산 시 Activation/Gradient covariance 일관성 확보
    """
    # Llama 모델의 경우 answer 시작 마커 필요
    if model_family and 'llama' in model_family.lower():
        # Llama의 경우 question_end_tag = " [/INST]"가 answer 시작을 알림
        answer_start_token = model_configs.get('question_end_tag', '')
        answer_text = answer_start_token + answer
    else:
        # 다른 모델은 기존 방식 (answer_tag 사용)
        answer_token = model_configs.get('answer_tag', '')
        answer_text = answer_token + answer
...
class TextDatasetQA(Dataset):
    def __init__(self, data_path, tokenizer, model_family, max_length=512, split = None, question_key='question', answer_key='answer', answer_only=True): #false로 바꾸기
        super(TextDatasetQA, self).__init__()
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.answer_only = answer_only  # Answer만 사용할지 여부
        # data_len = len(datasets.load_dataset(data_path, split)["train"])
        # self.data = datasets.load_dataset(data_path, split)["train"].select(range(min(100, data_len)))
        self.data = datasets.load_dataset(data_path, split)["train"]
        self.data = add_dataset_index(self.data)
    ...
    def __getitem__(self, idx):
        ...
        for answer in answers:
            if self.answer_only:
                # Answer만 사용 (Question 제거)
                converted_data = convert_answer_only_to_model_format(
                    self.tokenizer, self.max_length, answer, self.model_configs
                )
            else:
                # 기존 방식 (Question + Answer)
...
def custom_data_collator_with_indices(samples):
    ...
    labels = [s[1] for s in samples]
    attention_mask = [s[2] for s in samples]
    indices = [s[3] for s in samples]  # 실제 데이터셋 인덱스 사용
