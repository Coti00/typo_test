...
class CustomTrainerForgetting(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if self.loss_type == "grad_ascent":
            ...
            # Compute CE loss per token (원본 TNPO 방식)
            ce = nn.CrossEntropyLoss(ignore_index=-100, reduction='none')(
                shift_logits.view(-1, shift_logits.size(-1)),
                shift_labels.view(-1)
            )
            ce_oracle = nn.CrossEntropyLoss(ignore_index=-100, reduction='none')(
                shift_logits_oracle.view(-1, shift_logits_oracle.size(-1)),
                shift_labels.view(-1)
            )

            # NPO weights 계산 (TNPO 핵심)
            probs = (-ce).exp().detach()
            probs_oracle = (-ce_oracle).exp().detach()
            npo_weights = 2 * probs ** self.beta / (probs ** self.beta + probs_oracle ** self.beta + 1e-5)

            # SI score 마스킹
            fmask_shifted = forget_mask[..., 1:].contiguous()
            fmask_flat = fmask_shifted.view(-1)

            # Valid tokens: not -100
            valid_mask = (shift_labels.view(-1) != -100)

            # SI 점수가 0인 토큰에도 최소값 할당 (next token prediction이므로 모든 토큰 포함)
            si_weights = fmask_flat.clone()
            min_nonzero_weight = si_weights[si_weights > 0].min() if (si_weights > 0).any() else 1e-6
            si_weights[si_weights == 0] = min_nonzero_weight * 0.01  # 0인 토큰은 매우 작은 가중치

            # 유효한 토큰에 대해서만 SI 가중치 적용
            si_weights = si_weights[valid_mask]

            # TNPO loss with SI weighting (양수 loss로 forget 토큰 확률 감소)
            weighted_ce = npo_weights[valid_mask] * ce[valid_mask] * si_weights
            loss = weighted_ce.sum() / (si_weights.sum() + 1e-8)  # Normalize by SI weights
      
            print(f"tnpo_loss: {loss.item()}, valid_tokens: {valid_mask.sum().item()}, forget_tokens: {(fmask_flat[valid_mask] > 0).sum().item()}")
